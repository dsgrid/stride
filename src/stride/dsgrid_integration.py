...
def _bulk_register(
    ...
    skip_checks = False
    try:
        # TODO: dimension mapping details are unclear. We do not currently support mapping to
        # the project.
        if "__DSGRID_SKIP_CHECK_DATASET_TO_PROJECT_MAPPING__" not in os.environ:
            os.environ["__DSGRID_SKIP_CHECK_DATASET_TO_PROJECT_MAPPING__"] = "1"
            skip_checks = True
...
def _make_bulk_registration_config(
    ...
    return RegistrationModel(
        ...
        # TODO: we should really submit all datasets to project. There are unclear details
        # about dimensions to map.
        # DatasetSubmissionModel(
        #     dataset_id=x["dataset_id"],
        #     project_id=project_id,
        # ) for x in dataset_paths
        # ],
        dataset_submissions=[
            DatasetSubmissionModel(dataset_id="default__load_shapes", project_id=project_id)
        ],
    )
...
def make_mapped_datasets(
    ...
    submitter = ProjectQuerySubmitter(project, output_dir)
    for dataset_id in dataset_ids:
        query = make_query_for_standalone_dataset(
            project_id, dataset_id, None, column_type=ColumnType.DIMENSION_TYPES
        )
